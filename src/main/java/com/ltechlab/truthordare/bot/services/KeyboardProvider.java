...
import com.ltechlab.truthordare.bot.constants.mapkey.ActionMenu;
...
public class KeyboardProvider {
    ...
    public ReplyKeyboard keyboardStartNewGame() {
        return new KeyboardProvider().variablesKeyboard(
                ActionMenu.START_GAME_MENU_ITEM
        );
    }

    public ReplyKeyboard keyboardNextQuestion() {
        return new KeyboardProvider().variablesKeyboard(
                ActionMenu.NEXT_QUESTION_MENU_ITEM,
                ActionMenu.START_GAME_MENU_ITEM
        );
    }
...
//            List<String> collect = Arrays.stream(vars).filter(Objects::nonNull).toList();
//            KeyboardRow keyboardRow = new KeyboardRow();
